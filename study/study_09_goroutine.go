...
通道怎么关闭：
	...
	发送者、接收者：个人理解指的是goroutine
	    ...
	    从一个已关闭的channel中读取数据不会报错。只不过需要注意的是，接受者就不会被一个已关闭的channel的阻塞。而且接受者从关闭的channel中仍然可以读取出数据，
	只不过是这个channel的数据类型的默认值。我们可以通过指定接受状态位来观察接受的数据是否是从一个已关闭的channel所发送出来的数据。例如j, ok := <-c，则ok为false时，
	则代表channel已经被关闭。

		如下图这个例子，在协程关闭channel之前，主线程仍然会被这个协程所阻塞，而且读取数据时，注意状态位是true。当协程关闭channel之后，主线程仍然可以从channel中读取出int的默认值0，
    只不过状态变量变为了false，而且不再被阻塞，直到循环结束。
*/
